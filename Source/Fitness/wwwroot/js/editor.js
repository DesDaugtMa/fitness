document.addEventListener('DOMContentLoaded', () => {
    // ----------------------------------------------------------------------
    // 1. Daten vom Server (Razor Injection)
    // ----------------------------------------------------------------------
    const mockExercises = window.ExerciseData || [];
    ...
    function saveState() {
        if (typeof window.PlanId !== 'undefined') return; // Kein autosave im Edit Modus
        
        // Sichere Prüfungen falls Elemente kurzzeitig fehlen
        const planNameEl = document.getElementById('planName');
        const planDescEl = document.getElementById('planDesc');
        if(planNameEl) localStorage.setItem(STORAGE_KEY_NAME, planNameEl.value);
        if(planDescEl) localStorage.setItem(STORAGE_KEY_DESC, planDescEl.value || '');
        localStorage.setItem(STORAGE_KEY_DAYS, JSON.stringify(trainingDays));
    }

    function loadState() {
        // Edit Mode Injection via Razor
        if (typeof window.InitialPlanData !== 'undefined' && window.InitialPlanData) {
            const elName = document.getElementById('planName');
            if (elName) elName.value = window.InitialPlanData.planName || '';

            const elDesc = document.getElementById('planDesc');
            if (elDesc) elDesc.value = window.InitialPlanData.planDesc || '';

            if (window.InitialPlanData.days && window.InitialPlanData.days.length > 0) {
                return window.InitialPlanData.days;
            }
        }
        ...
        if (savedDesc) {
            const el = document.getElementById('planDesc');
            if(el) el.value = savedDesc;
        }
        ...
        trainingDays.forEach((loadedDay, index) => {
                ...
                callbackOnCreateTemplates: function(template) {
                    return {
                        choice: (classNames, data) => {
                            ...
                            const tags = muscles.map(m => `<span class="badge bg-secondary me-1" style="font-size: 0.8rem;">${m}</span>`).join('');
            ...
            selectEl.addEventListener('change', function(e) {
                ...
                if (selectedId) {
                    ...
                    // Entferne die aktive Auswahl im Dropdown, damit die exakt selbe Übung noch einmal geklickt werden kann
    ...
    function renderDayExercises(dayId) {
        ...
        if (day.exercises.length === 0) {
            ...
            day.exercises.forEach((ex, index) => {
                const tagsStr = ex.muscle.map(m => `<span class="badge bg-secondary me-1" style="font-size: 0.8rem;">${m}</span>`).join('');
                const itemHtml = `
                    <div class="swipe-wrapper" data-wrapper-id="${ex.instanceId}">
                        ...
                        <div class="exercise-item p-3" data-instance-id="${ex.instanceId}">
                            <div class="d-flex align-items-center mb-3 position-relative">
                                ...
                                <button type="button" class="btn btn-link text-danger p-2 ms-2 d-none d-lg-block" onclick="removeExerciseGlobal('${dayId}', '${ex.instanceId}')" title="Übung entfernen" style="text-decoration: none;">
    ...
    if(form) {
        form.addEventListener('submit', function(e) {
            ...
            if (isValid) {
                ...
                saveBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Speichern...';
                saveBtn.disabled = true;
                
                const payload = {
                    planName: planNameInput.value,
                    planDesc: planDescInput.value,
                    days: trainingDays
                };

                const fetchUrl = typeof window.PlanId !== 'undefined' ? `/TrainingPlans/Edit/${window.PlanId}` : '/TrainingPlans/Save';
                ...
                .then(data => {
                    if(data.success) {
                        // Lösche localStorage wenn man fertig ist
                        ...
                        setTimeout(() => {
                            window.location.href = '/'; // TODO: Redirect to plans list
                        }, 1500);
                    } else {
                ...
                .catch(error => {
                    globalError.innerHTML = '<strong>Achtung:</strong><br>' + error.message;
